(() => {
    ...
    if (burger && menu) burger.addEventListener("click", (() => {
        ...
        script_body.classList.toggle("_locked");
    }));
    const script_filter = document.querySelector(".filter");
    if (script_filter) {
        const items = script_filter.querySelectorAll(".block-filter");
        items.forEach((item => {
            item.addEventListener("click", (event => {
                item.querySelector(".block-filter__dropdown").classList.toggle("_active");
                item.querySelector(".block-filter__icon").classList.toggle("_active");
                if (event.target.classList.contains("block-filter__  item")) item.querySelector(".block-filter__value").textContent = event.target.textContent;
            }));
        }));
    }
